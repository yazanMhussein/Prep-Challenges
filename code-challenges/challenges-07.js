...
const objLat = (obj) => {
    let firstChar =obj.firstName.toUpperCase().charAt(0);
    let lastChar =obj.lastName.toUpperCase().charAt(0);
    let fName=firstChar.concat(obj.firstName.slice(1));
    let lName=lastChar.concat(obj.lastName.slice(1));
...
const cvFormatter = (arr) => {
    ...
    for (let obj of arr) {
        ...
        if (obj.yearsOfExperience && obj.yearsOfExperience > 1) {
            newArr.push({ fullName, tech: obj.tech });
        }
    }
    return newArr;
};
