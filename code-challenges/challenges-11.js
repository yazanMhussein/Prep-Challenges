...
function square(arr) {
   const newvalue = arr.map((value) => value*=value);
    return newvalue;
}
// 111
// -------------------------------------------------------------------------------------------------------
...
function gradesAvg(arr) {
    ...
    return arr.map(student => {
        // start with a 0 sum value
        let total = 0;
        // for each student we loop through their grades list
        (student.gradsList).forEach((number)=>{
            // add each grade to the total sum value
            total +=number;
        });
        // calculate the average
        const avg = total/student.gradsList.length;
        // add the average to the student object and return the new student object
        student.avg = avg;
        return student;
    })
